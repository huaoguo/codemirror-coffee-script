(function () {
  ...
  CodeMirror.defineSimpleMode('coffeescript', {
    ...
    start: [
      ...
      { regex: /[a-zA-Z]\w*::[a-zA-Z]\w*/, token: 'attribute property' }, // coffeescript prototype
      { regex: /[a-zA-Z]\w*:/, token: 'attribute property' }, // property def
      { regex: /[A-Z]\w*/, token: 'variable-2 variable-3' }, // capital variable always be class or module
      { regex: /(\([^\(\)]*\))\s*(?=->)/, token: 'def' }, // method signature
      { regex: /(?:@|this\.)[a-z\.]\w*/, token: 'property' }, // property access
      { regex: '@|this', token: 'keyword' }, // this keyword
      { regex: atoms.join('|'), token: 'atom' }, // atoms
